...
# 1. SET PAGE CONFIG (Lệnh Streamlit bắt buộc phải khai báo đầu tiên)
st.set_page_config(
    layout="wide",
...
with st.sidebar:
    ...
    st.caption("Mô hình: Random Forest Classifier (Theo Notebook)")
...
st.title("🛡️ Hệ thống Phát hiện Giao dịch Gian lận & Rủi ro tín dụng")
st.caption("Ứng dụng hỗ trợ phân tích dữ liệu giao dịch, đánh giá mô hình học máy Random Forest và dự báo trực tuyến rủi ro gian lận/nợ xấu (Cột mục tiêu: 'default').")

# Kiểm tra trạng thái dữ liệu đầu vào hành vi người dùng
if uploaded_file is None:
    st.info("💡 Vui lòng tải lên tệp dữ liệu huấn luyện (.csv hoặc .xlsx) ở thanh cấu hình bên trái để bắt đầu.")
    st.stop()
...
if df_raw is None:
    st.error("Không thể xử lý định dạng tệp này. Vui lòng kiểm tra lại cấu trúc file.")
    st.stop()

# Kiểm tra sự tồn tại của biến mục tiêu 'default'
if 'default' not in df_raw.columns:
    st.error("❌ Tệp dữ liệu thiếu cột mục tiêu 'default'. Vui lòng chọn đúng tệp cấu trúc.")
    st.stop()

st.caption(f"📁 Đang dùng tệp: `{uploaded_file.name}` | Quy mô dữ liệu: {df_raw.shape[0]} dòng, {df_raw.shape[1]} cột.")
st.divider()
...
if missing_features:
    st.error(f"❌ Thiếu các cột tính năng bắt buộc sau trong dữ liệu đầu vào: {missing_features}")
    st.stop()
...
if btn_train:
    with st.spinner("🔄 Đang xử lý dữ liệu và huấn luyện mô hình..."):
        ...
        st.success("🎉 Huấn luyện mô hình thành công! Hãy xem kết quả chi tiết ở các Tab bên dưới.")
...
with tab_overview:
    st.subheader("🗂️ Phân tích thống kê và Cấu trúc tệp dữ liệu")
    ...
    st.markdown("### 🗄️ Xem trước dữ liệu thô (5 dòng đầu tiên)")
    st.dataframe(df_raw.head(5), use_container_width=True)
    
    # Thống kê mô tả tập dữ liệu mô hình
    st.markdown("### 📉 Chỉ số mô tả các biến đặc trưng đưa vào mô hình")
...
with tab_viz:
    st.subheader("📊 Trực quan phân phối tính năng & Biến mục tiêu")
    ...
    st.markdown("### 📊 Biểu đồ phân phối chi tiết các biến đầu vào")
    ...
    if selected_features:
        ...
        num_cols_layout = 2
        cols = st.columns(num_cols_layout)
        ...
        st.warning("Vui lòng lựa chọn ít nhất một biến đầu vào để trực quan hóa.")
...
with tab_eval:
    ...
    if 'trained_model' not in st.session_state:
        st.info("💡 Vui lòng thiết lập cấu hình tham số và bấm nút **[🚀 Huấn luyện mô hình]** ở thanh Sidebar bên trái để hiển thị kết quả kiểm định.")
    else:
        ...
        with col_right:
            st.markdown("### 🌲 Độ quan trọng của các biến (Feature Importance)")
...
with tab_predict:
    st.subheader("🔮 Dự báo rủi ro gian lận giao dịch cho dữ liệu mới")
    
    if 'trained_model' not in st.session_state:
        st.info("💡 Vui lòng hoàn thành bước huấn luyện mô hình để kích hoạt tính năng dự báo.")
    else:
        ...
        if mode == "👉 Nhập trực tiếp từng bản ghi":
            st.markdown("### 📝 Điền thông số các biến đặc trưng giao dịch")
                ...
                for idx, col_name in enumerate(feature_cols):
                    ...
                    with current_p_col:
                        user_inputs[col_name] = st.number_input(
                            ...
                            min_value=float(min_values[col_name] * 5),
            ...
            st.markdown("### 📂 Tải lên file chứa cấu trúc danh sách biến đầu vào")
            st.caption("Lưu ý: File tải lên bắt buộc phải chứa đầy đủ các cột từ `X_1` tới `X_14`.")
            ...
            if predict_file is not None:
                ...
                if df_predict_raw is not None:
                    ...
                    if missing_p_features:
                        st.error(f"❌ File tải lên không hợp lệ. Thiếu các cột tính năng bắt buộc sau: {missing_p_features}")
                    else:
                        ...
                        st.markdown("### 📊 Danh sách kết quả dự báo tổng hợp")
